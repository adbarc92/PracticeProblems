...
function BinaryHeap() {
  ...
  this._swap = function (parentIndex, childIndex) {
...
BinaryHeap.prototype.getParentIndex = function (index) {
  ...
  const value = Math.floor((index - 1) / 2);
...
BinaryHeap.prototype.fixHeap = function () {
  let currentIndex = this._heap.length - 1;
  while (
    this._compare(
      this._heap[currentIndex],
      this._heap[this.getParentIndex(currentIndex)]
    )
  ) {
...
BinaryHeap.prototype.removeRoot = function () {
  ...
  while (currentIndex !== this._heap.length - 1) {
    // Check which child has a lower value
    const [firstChild, secondChild] = this.getChildrenIndices(
      currentIndex
    );
    const swapIndex = this._compare(firstChild, secondChild)
      ? firstChild
      : secondChild;
    // Swap current node with the lower value
    this._swap(currentIndex, swapIndex);
    // Set currentIndex to lower value
