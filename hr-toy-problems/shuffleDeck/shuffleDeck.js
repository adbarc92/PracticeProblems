...
const humanShuffle = (deck) => {
  const m = Math.floor(deck.length / 2);
  const l = deck.slice(0, m);
  const r = deck.slice(m);
  const a = [];
  for (let i = 0; i < 7; i++) {
    while (l.length && r.length) {
      a.push(l.splice(0, 1)[0]);
      a.push(r.splice(0, 1)[0]);
    }
  }
  return a;
};
...
const shuffleDeck = function (deck) {
  ...
  // debugger;
  while (eDeck.length) {
    const i = getRandomInt(eDeck.length);
    rDeck.push(eDeck.splice(i, 1)[0]);
  }
  return rDeck;
};
...
// const a = shuffleDeck(orderedDeck());
// const b = 0;
// debugger;
